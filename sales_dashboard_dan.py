...
def safe_plot(plot_func, title="", is_heatmap=False, *args, **kwargs):
...
def main():
            ...
            if st.button("Generate Forecast"):
                ...
                try:
                    ...
                    if not wa_data.empty:
                        ...
                        wa_growth_rate = wa_sales_series.pct_change().mean()
                    else:
                        wa_growth_rate = 0.05  # Default growth rate if no WA data
                    ...
                    for branch in selected_branches:
                        ...
                        if model_choice == "Simple Growth":
                            ...
                            try:
                                sales_series = branch_df.groupby('Weeks Since Start')['Total'].sum()
                                ...
                                st.warning(f"ARIMA failed for {branch}: {str(e)}. Using simple growth.")
                                initial_sales = branch_df.groupby('Weeks Since Start')['Total'].sum().iloc[0]
                                projected = initial_sales * (1 + (growth_rate/100)) ** np.arange(1, forecast_weeks + 1)
                        
                        # Apply WA's growth rate as target
                        initial_sales = branch_df.groupby('Weeks Since Start')['Total'].sum().iloc[0]
                        projected_wa = initial_sales * (1 + wa_growth_rate) ** np.arange(1, forecast_weeks + 1)
                        
                        forecast_data.append(pd.DataFrame({
                            'Week': np.arange(1, forecast_weeks + 1),
                            'Branch': branch,
                            'Projected Sales': projected,
                            'Projected Sales (WA Growth)': projected_wa
                        }))
                    ...
                    if forecast_data:
                        ...
                        fig = px.line(forecast_df,
                                     x='Week',
                                     y=['Projected Sales', 'Projected Sales (WA Growth)'],
                                     color='Branch',
    ...
    with tabs[1]:
        ...
        try:
            ...
            # Quarterly Spend - Only shown once
    ...
    with tabs[2]:
        ...
        try:
            ...
            # Growth Rate Analysis - Fixed Version
    ...
    # EXPORT TAB - Fixed Version with Kaleido handling
    with tabs[3]:
        ...
        try:
            ...
            # Plot Export with Kaleido check
            try:
                # Check for kaleido
                try:
                    import kaleido
                    ...
                    st.warning("For PNG exports, please install Kaleido:")
                ...
                if KALEIDO_INSTALLED:
                    ...
                    st.download_button(
                        label="Download Time Series Plot (PNG)",
                        data=plot_bytes,
                        file_name='time_series_plot.png',
                        mime='image/png'
                    )
                else:
                    st.warning("PNG export requires Kaleido. Install with: pip install -U kaleido")
                
                # SVG Export (works without Kaleido)
                plot_bytes_svg = ts_fig.to_image(format="svg")
                st.download_button(
                    label="Download Time Series Plot (SVG)",
                    data=plot_bytes_svg,
                    file_name='time_series_plot.svg',
                    mime='image/svg+xml'
                )
